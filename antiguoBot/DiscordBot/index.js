...
client.on("messageCreate", async function(message) { 
    ...
    if (command === "ping") {
        ...
        //fetch prueba
        const response = await fetch("http://quizbot.alumnes.inspedralbes.cat:7005/preguntas/api");
        ...
        const pregunta = data.preguntas[i].pregunta;
        const array = data.preguntas[i].respuestas;
        ...
        const respuestaCorrecta = data.preguntas[i].rc;
        ...
        const exampleEmbed = new MessageEmbed()
                ...
                .setImage(data.preguntas[i].imagen)
            ...
            switch(reaction.emoji.name){
                ...
                case '1️⃣':
                    if(respuesta1 === respuestaCorrecta){
                        ...
                        message.channel.send(`Respuesta CORRECTA de ${user.username}`);
                        ...
                        message.channel.send(`Respuesta INCORRECTA de ${user.username}`);
                    }
        ...
        collector.on('end', collected => {
            const timeTaken = sentEmbed.createdTimestamp - message.createdTimestamp +1000;
            message.channel.send(`En ${timeTaken} ms`) 
            
            // const respuestaTiempo =
            // const tiempoRespuesta = parseInt(Discord.ReactionCollector.createdTimestamp) - timeTaken ;
            // const tiempoRespuesta = collected ;
            // console.log(collected.Collection[1])

            // message.channel.send(`Se ha cogido ${collected.size} item`);
            // const tiempoRespuesta = timeTaken - ;
        });
